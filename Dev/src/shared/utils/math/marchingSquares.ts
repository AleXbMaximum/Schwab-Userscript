...
// Returns fractional grid coordinates suitable for direct canvas mapping.
 ...
 *
 * @param grid   - `grid[row][col]` scalar values; `null` entries are treated as
 *                 below any threshold (no contour through missing data).
...
export function marchingSquares(
  ...
  for (let r = 0; r < rows - 1; r++) {
    for (let c = 0; c < cols - 1; c++) {
      ...
      // Skip cells with any null corner
      if (tl == null || tr == null || bl == null || br == null) continue;

      // Build 4-bit case index (bit set = above threshold)
 ...
 * Uses the Heckbert nice-number algorithm to produce round values.
 */
export function pickContourLevels(
  ...
  for (let v = start; v < maxVal; v += step) {
    const rounded = Math.round(v * 1e6) / 1e6; // avoid float noise
