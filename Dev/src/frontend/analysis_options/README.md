...
│   ├── savedViewTypes.ts      #   SavedViewState, OptionsSavedView, TimestampSavedView
│   ├── savedViewSerializer.ts #   encode/decode, copy-out formatters
│   └── savedViewRepository.ts #   IndexedDB read/write
├── store/                     # Pub-sub state management
│   ├── OptionsViewStore.ts    #   Store creation, auto-refilter on patch
│   ├── filters.ts             #   Chain filtering (scope, local window, liquidity, strikes)
│   └── selectors.ts           #   Derived selectors (alerts, view state projection)
├── controls/                  # UI controls for view filters
│   ├── OptionsViewControls.ts #   Expiry/scope/strike/liquidity/basis selects
│   ├── controlPresets.ts      #   Constants for control options
│   └── controlFormatters.ts   #   Expiry label formatting helpers
├── focus/                     # Global focus-strike overlay state
│   ├── focusStrike.ts         #   PubSub state for focused strikes
